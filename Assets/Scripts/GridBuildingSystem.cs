...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(lookPosition != Vector3.zero) //포지션이 아닐때
        {
    ...
    private void CreateGrid()
    {
        ...
        girdCenter.y = 0;
        transform.position = girdCenter = new Vector3(width * cellSize / 2.0f, 0, height * cellSize / 2); //그리드 중심으로 이동시킨다.
    ...
    private Vector3 GetLookPosition()
    {
        if (playerController.isFirstPerson) //플레이어가 1인칭 모드
        {
            ...
            {
                Debug.DrawRay(ray.origin, ray.direction * hitInfo.distance, Color.red); //Scene 창에서 빨간색 신으로 ray를 보여준다.
        ...
        else
        {
            ...
            Vector3 rayOrigin = characterPosition + Vector3.up * 1.5f + characterForward * 0.5f; //레이를 시작 위치에 설정
            Vector3 rayDirection = (characterForward = Vector3.up).normalized; //레이를 쏠 방향 설정 후 규정화 (0-1)

            Ray ray = new Ray(rayOrigin, rayDirection); //만든 위치와 방향으로 ray를 생성
            if (Physics.Raycast(ray, out RaycastHit hitInfo, maxBuildDistance)) //설정한 ray에 물체가 있을 경우
            {
    ...
    //선택된 셀을 하이라이트하는 함수
    private void HighlightCell(Vector3Int gridPositon)
    {
        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z <height; z++)
        ...
        GridCell cell = cells[gridPositon.x, gridPositon.z];
        GameObject highlightObject =
            cell.Building != null ? cell.Building : transform.GetChild(gridPositon.x * height + gridPositon.z).gameObject;
        highlightObject.GetComponent<Renderer>().material.color = cell.IsOccupied ? Color.red : Color.white;
    }
    ...
    private void PlaceBuilding(Vector3Int gridPosition)
    {
        ...
        {
            Vector3 woridPositon = grid.GetCellCenterWorld(gridPosition); //셀의 위치를 가져온다.
            GameObject building = Instantiate(buildingPrefabs, woridPositon, Quaternion.identity); //건물을 생성한다.
    ...
    private void RemoveBuiding(Vector3Int gridPosition)
    {
    ...
    private void OnDrawGizmos() //Gizmo를 표시해주는 함수
    {
        ...
        for (int x = 0; x < width; x++) 
        {
            for(int z = 0; z < height; z++)
            {
                Vector3 cellCenter = grid.GetCellCenterWorld(new Vector3Int(x, 0, z)); //그리드 주임 점을 가져온다.
